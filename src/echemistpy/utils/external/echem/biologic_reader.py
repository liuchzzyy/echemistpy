...
def MPTfile(file_or_path, encoding="latin1"):
...
def loop_from_file(file: str | Path, encoding: str = "latin1"):
    """
...
def timestamp_from_file(file: str | Path, encoding: str = "latin1"):
    """
...
class MPRfile:
    ...
    def __init__(self, file_or_path, error_on_unknown_column: bool = True):
        ...
        if isinstance(file_or_path, (str, Path)):
            ...
            loop_file = path_str[:-4] + "_LOOP.txt"  # loop file for running experiment
            log_file = path_str[:-1] + "l"  # log file for runnning experiment
        else:
        ...
        try:
            ...
            if maybe_loop_module:
                ...
                if loop_file and Path(loop_file).is_file():
...
class BiologicReadError(Exception):
    """Raised when a BioLogic file cannot be parsed."""


class BiologicMPTReader:
    """Reader for BioLogic .mpt and .mpr files."""
    ...
    def read(self) -> tuple[RawData, RawDataInfo]:
        ...
        if not self.path_to_file.exists():
            raise BiologicReadError(f"File not found: {self.path_to_file}")

        suffix = self.path_to_file.suffix.lower()
        if suffix == ".mpt":
            return self._read_mpt()
        elif suffix == ".mpr":
            return self._read_mpr()
        else:
            raise BiologicReadError(
                "Only BioLogic .mpt or .mpr exports are supported by this reader.",
            )

    def _read_mpt(self) -> tuple[RawData, RawDataInfo]:
        try:
            mpt_array, comments = MPTfile(self.path_to_file)
        except Exception as exc:
            raise BiologicReadError(f"Failed to parse MPT file: {exc}") from exc

        # Convert numpy record array to xarray Dataset
        data_vars = {}
        coords = {"row": np.arange(len(mpt_array))}
        
        for name in mpt_array.dtype.names:
            data_vars[name] = (("row",), mpt_array[name])

        dataset = xr.Dataset(data_vars, coords=coords)
        
        # Extract metadata from comments
        meta = self._parse_comments(comments)
        meta["header"] = comments

        return RawData(data=dataset), RawDataInfo(meta=meta)

    def _read_mpr(self) -> tuple[RawData, RawDataInfo]:
        try:
            mpr = MPRfile(self.path_to_file)
        except Exception as exc:
            raise BiologicReadError(f"Failed to parse MPR file: {exc}") from exc
        ...
        for column_name in column_names:
            data_vars[column_name] = (("row",), data[column_name])

        dataset = xr.Dataset(data_vars, coords=coords)

        meta = {
            "start_date": mpr.startdate,
            "end_date": mpr.enddate,
            "timestamp": getattr(mpr, "timestamp", None),
            "version": mpr.version,
            "modules": mpr.modules,
        }
        if mpr.loop_index is not None:
            meta["loop_index"] = mpr.loop_index
